...
from mmcls.core.optimizer import build_optimizers
...
def train_model_bilevel(model,
    ...
    # 将训练数据集划分成数量相同的两部分，分别供权重和阈值训练使用
    assert len(dataset) == 1
    n_train = len(dataset[0])
    split = n_train // 2
    # 确保划分成的两个子集样本数量相同
    assert split * 2 + 1 == n_train
    datasets = list(torch.utils.data.random_split(
        dataset[0], [split, split, 1], generator=torch.Generator().manual_seed(33)))
    datasets.pop(-1)

    # 结构参数和网络权重各使用一半的训练集数据
    data_loaders = [
        build_dataloader(
            ds,

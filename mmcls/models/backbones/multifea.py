...
from .binary_utils.functions import build_act
from .binary_utils.multifea_blocks import MultiFeaBlock, MultiFeaSimBlock


@BACKBONES.register_module()
class MultiFea(BaseBackbone):
    """BNN MultiFea backbone.
    ...
    arch_settings = {
        'resnet18': (MultiFeaBlock, (2, 2, 2, 2)),
        'resnet18_sim': (MultiFeaSimBlock, (2, 2, 2, 2)),
    }
    ...
    def __init__(self,
                 ...
                 **kwargs):
        super(MultiFea, self).__init__(init_cfg)

        self.arch = arch
        if self.arch not in self.arch_settings:
            raise KeyError(f'invalid arch {arch} for MultiFea network')
        self.binary_type = binary_type
        self.stem_act_name = stem_act
        self.block_act = block_act
        self.fexpand_num = fexpand_num
        self.fexpand_mode = fexpand_mode
        self.fexpand_thres = fexpand_thres
            ...
            for j in range(num_blocks):
                ...
                self.layers.append(
                    self.block(
                        ...
                        nonlinear=block_act,
                        fexpand_num=fexpand_num,
                        fexpand_mode=fexpand_mode,
                        fexpand_thres=fexpand_thres,
                        **kwargs
                    ))
    ...
    # def _build_act(self):
    #     act_name = self.stem_act_name
    #     if act_name == 'prelu':
    #         return nn.PReLU(self.stem_channels)
    #     elif act_name in act_name_map:
    ...
    def init_weights(self):
        super(MultiFea, self).init_weights()
    ...
    def forward(self, x):
        ...
        if self.block == MultiFeaSimBlock:
            cos_sim = 0
            for layer in self.layers:
                layer_cos_sim, x = layer(x)
